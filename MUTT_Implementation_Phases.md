...
## Phase 1: Models & Configuration (~200 lines)

**Objective**: Build the foundational data structures and configuration system.

**Files to Create**:
1. `mutt/models/message.py` - Message dataclasses
2. `mutt/models/rules.py` - Alert rule dataclasses
3. `mutt/config.py` - YAML config loader
4. `config/mutt_config.yaml` - Configuration file
5. `config/alert_rules.yaml` - Alert rules file

**Dependencies**: None (standard library only)

**Testing**: 
```bash
python -c "from mutt.models.message import Message, SNMPTrap; print('Phase 1 OK')"
```
...
- Sample config and alert rules files
...
**Deliverables**:
- Complete SQLite schema (messages, devices, archives, snmp_messages, syslog_messages tables)
...
## Phase 3: Listeners (~300 lines)

**Objective**: Build SNMP and Syslog message receivers.

**Files to Create**:
1. `mutt/listeners/base.py` - Abstract listener base class
2. `mutt/listeners/snmp_listener.py` - SNMP trap listener
...
**Notes for DeepSeek**:
- Use `pysnmp` library (may need to mock if not available)
- Use standard library `socket` for syslog
- Syslog regex for RFC 3164: `<(\d+)>(\w+\s+\d+\s+\d+:\d+:\d+)\s+(\S+)\s+(\S+?)(?:\[(\d+)\])?:\s*(.*)`
- SNMP listener can be simplified for home use (basic v2c/v3 support)
...
│   │   ├── message.py                   [Phase 1]
│   │   ├── rules.py                     [Phase 1]
│   │   └── credentials.py               [Phase 1 - stub for future]
│   └── utils/
│       ├── __init__.py
