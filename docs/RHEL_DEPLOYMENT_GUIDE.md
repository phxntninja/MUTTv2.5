# MUTT v2.5 RHEL Deployment Guide

This guide provides detailed instructions for deploying the MUTT v2.5 application on a Red Hat Enterprise Linux (RHEL) server. The primary deployment method uses `systemd` to manage the application services.

A deployment script, `deploy_rhel.sh`, is provided to automate many of these steps. You can run the script directly or follow the manual steps below for a more controlled installation.

## 1. Prerequisites

### 1.1. System Dependencies
-   A RHEL Server (RHEL 8 or newer is recommended).
-   `sudo` or root access.
-   Python v3.10.

The `deploy_rhel.sh` script assumes Python 3.10 is already installed and available as `python3.10`. You may need to install it first. For RHEL 8/9, you can use `dnf`:
```bash
sudo dnf install python3.10
# Also install the venv module
sudo dnf install python3.10-venv
```

### 1.2. External Services
The application requires the following external services to be running and accessible from the RHEL host:
...
```bash
sudo useradd --system --home-dir /opt/mutt --shell /bin/bash mutt
```
...
# The useradd command should create the 'mutt' group automatically
...
RHEL systems typically use `firewalld`. The deployment script omits this step, so you must do it manually. Allow access to the web UI port (8090/tcp).
...
The `systemd` service files are identical to the ones used for the Ubuntu deployment. They are included here for your reference.
